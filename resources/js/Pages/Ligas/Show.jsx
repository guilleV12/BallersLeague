...
const Show = ({ auth, user, liga, equipos, userAdmin, arbitros, users, miLiga, calendario, fechas }) => {
  return (
    <AuthenticatedLayout auth={auth} user={user} liga={liga} miLiga={miLiga}>
        ...
        <div className='w-2/3 ml-[10%] flex justify-center min-h-screen'>
                {liga[0] ? (
                  <CardLigaShow liga={liga[0]} user={user} equipos={equipos} userAdmin={userAdmin[0]} arbitros={arbitros} users={users} calendario={calendario} fechas={fechas}></CardLigaShow>
                ) : (
