...
const Show = ({ auth, user, liga, equipos, userAdmin, arbitros, users, miLiga, calendario, fechas, jugadores, partidos, jugadorPartido, rol }) => {
  return (
    <AuthenticatedLayout auth={auth} user={user} liga={liga} miLiga={miLiga}>
        ...
        <div className='flex justify-center lg:items-center w-full lg:w-[78%] h-fit  lg:ml-[15.5rem] xl:ml-[16rem]'>
                {liga ? (liga[0] ? (
                  <CardLigaShow rol={rol} jugadorPartido={jugadorPartido} partidos={partidos} jugadores={jugadores} liga={liga[0]} user={user} equipos={equipos} userAdmin={userAdmin[0]} arbitros={arbitros} users={users} calendario={calendario} fechas={fechas}></CardLigaShow>
                ) : (
