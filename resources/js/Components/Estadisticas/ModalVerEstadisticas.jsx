...
export const ModalVerEstadisticas = ({ users, partidos, fechaPartidoVer, setShowAlert, setTituloAlert, closeModalVerEstadisticas, equipos, jugadores, jugadorPartido, arbitros }) => {
  ...
  <div className={`fixed inset-0 flex items-center justify-center z-50 mt-[5%] mb-[1%]`}>
    <div className="bg-gray-100 w-full max-w-3xl max-h-full rounded-l-lg overflow-y-auto">
      ...
      <table className="w-full border border-separate border-black rounded-lg p-5 bg-white">
        <thead>
            <tr>
            <th className="text-left text-xl font-bold border-b" colSpan="2">Resumen del Partido</th>
            </tr>
        </thead>
        <tbody>
            <tr>
            <td className="text-lg font-bold pt-3">Fecha:</td>
            <td className="text-lg">{fechaPartidoVer.fecha} {fechaPartidoVer.horario}.</td>
            </tr>
            <tr>
            <td className="text-lg font-bold">Árbitros:</td>
            <td className="text-lg">{userArbitro1.nombre} {userArbitro1.apellido} / {userArbitro2.nombre} {userArbitro2.apellido}.</td>
            </tr>
            <tr>
            <td className="text-lg font-bold">Ganador:</td>
            <td className="text-lg">{equipoGanador.nombre}.</td>
            </tr>
        </tbody>
        </table>
        ...
        <table className="w-full border border-separate border-black rounded-lg p-5 mt-3 bg-white">
          <thead>
            <tr>
              <th className="text-left text-xl font-bold border-b" colSpan="3">Resultado del Partido</th>
            </tr>
            <tr>
              <th className="text-lg font-bold border-r pt-3">{equipo1.nombre}</th>
              <th className="text-lg font-bold border-r pt-3">Marcador</th>
              <th className="text-lg font-bold pt-3">{equipo2.nombre}</th>
            </tr>
          </thead>
          <tbody>
            <tr>
        ...
        <table className="w-full border border-separate border-black rounded-lg p-5 mt-3 bg-white">
          <thead>
            <tr>
              <th className="text-left text-xl font-bold border-b" colSpan="2">Puntos Individuales</th>
            </tr>
          </thead>
          <tbody>
            <tr>
                      ...
                      jugadorObj.id === jugador.jugador_id && jugadorObj.equipo_id === equipo1.id ? (
                        <div className="grid grid-cols-2 justify-start items-center my-3" key={`jugador_${jugadorObj.id}_equipo_1`}>
                          ...
                          <div className="flex flex-col justify-start">
                            <span className="text-md font-semibold">{jugadorObj.nombre + ' ' + jugadorObj.apellido}</span>
                            <span className="text-md font-semibold">Puntos: {jugador.puntos_anotados}</span>
                          </div>
                        </div>
                      ) : ''
                    )
        ...
        <div className="flex items-center justify-center mt-4">
          <PrimaryButton className="ml-2" onClick={closeModalVerEstadisticas}>
            Cerrar
          </PrimaryButton>
        </div>
      </div>
