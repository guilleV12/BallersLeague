  ...
  return (
   ...
   <table className="text-gray-500 dark:text-gray-400 w-full h-min-screen ">
            <thead className="text-base text-left font-semibold text-white bg-black dark:bg-gray-700 dark:text-gray-400">
                <tr className={`grid ${(userAuth && userAdmin.id === userAuth.id)?'grid-cols-3 md:grid-cols-4':'grid-cols-2 md:grid-cols-3'}`}>
                ...
                {arbitros.map((arbitro) => (
                    users.map((usuario) => ( (usuario.id === arbitro.id_user) ) ? (
                      (arbitro.deshabilitado === 0) ? (
                        <tr key={arbitro.id} className={`bg-white border-b grid ${(userAuth && userAdmin.id === userAuth.id)?'grid-cols-3 md:grid-cols-4':'grid-cols-2 md:grid-cols-3'} dark:bg-gray-900 dark:border-gray-700 text-sm rounded-b-lg`}>
