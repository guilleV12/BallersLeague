  ...
  return (
   ...
   <table className="text-gray-500 dark:text-gray-400 w-full h-min-screen ">
            <thead className="text-base text-left font-semibold text-white bg-black dark:bg-gray-700 dark:text-gray-400">
                ...
                <tr className={`grid ${(userAuth && userAdmin.id === userAuth.id)?'grid-cols-4':'grid-cols-3'}`}>
                  <th scope="col" className="px-6 py-1 hidden md:flex justify-center">
                    Nombre
                  </th>
                  <th scope="col" className="px-6 py-1 hidden md:flex justify-center">
                    DNI
                ...
                {arbitros.map((arbitro) => (
                    users.map((usuario) => ( (usuario.id === arbitro.id_user) ) ? (
                      (arbitro.deshabilitado === 0) ? (
                        <tr key={arbitro.id} className={`bg-white border-b grid ${(userAuth && userAdmin.id === userAuth.id)?'grid-cols-4':'grid-cols-3'} dark:bg-gray-900 dark:border-gray-700 text-sm rounded-b-lg`}>
                            <td className="flex justify-center items-center text-gray-900">{usuario.nombre} {usuario.apellido}</td>
                            <td className="flex justify-center items-center text-black">{usuario.dni}</td>
